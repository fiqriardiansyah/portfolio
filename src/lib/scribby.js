...
class Scribby {
  ...
  getMousePosition(ev) {
    ...
    if (ev.pageX && ev.pageY) {
      const coord = { x: ev.pageX - rect.left, y: ev.layerY }
      return coord
    }
  ...
  undo() {
    if (!this.svgEl.lastElementChild || (this.svgEl.lastElementChild.nodeName != 'path' && this.svgEl.lastElementChild.nodeName != 'text'))
      return false
